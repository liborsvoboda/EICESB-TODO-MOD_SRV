...
(function () {
  ...
  window.sc = window.spellcheck = {
    ...
    excludedWords:
      ...
      "www,com,br,nbsp,---,--,startuml,enduml",
            ...
            setTimeout(function (line, isLast) {
              ...
              if (lineText && lineText.length > 2 && lineText.substr(0, 3) === "```") {
                ...
                if (lineText && lineText.length > 6 &&
                    (lineText.startsWith("@startuml") || lineText.startsWith("@enduml"))) {

                  if (lineText.trim() == "@startuml")
                      isPlantUml= true;
                  else if(lineText.trim() == "@enduml")
                      isPlantUml = false;                                        
              }

              if (!isCodeBlock && !isFrontMatter && !isPlantUml) {
  ...
  function misspelled(line, skipWord) {
    ...
    for (var wordIndex in words) {
      ...
      // only use words without special characters
        ...
        i += word.length + 1 + beginCharoffset + endCharoffset;
    }
    return bads;
  }
})();
